...
def solve_(state):
    newrubiksmovequeue = []
    for i in step1(state)[0]:
        newrubiksmovequeue.append(i)
    for i in step2(step1(state)[1])[0]:
        newrubiksmovequeue.append(i)
    for i in step3(step2(step1(state)[1])[1])[0]:
        newrubiksmovequeue.append(i)
    for i in step4(step3(step2(step1(state)[1])[1])[1])[0]:
        newrubiksmovequeue.append(i)
    for i in step5(step4(step3(step2(step1(state)[1])[1])[1])[1])[0]:
        newrubiksmovequeue.append(i)
    for i in step6(step5(step4(step3(step2(step1(state)[1])[1])[1])[1])[1])[0]:
        newrubiksmovequeue.append(i)
    for i in step7(step6(step5(step4(step3(step2(step1(state)[1])[1])[1])[1])[1])[1])[0]:
        newrubiksmovequeue.append(i)
    return newrubiksmovequeue
gluPerspective(45, dimensions[0] / dimensions[1], 0.1, 2000*3/size)
glTranslatef(-2,0,-30)
z=-30
up=True
rotatekey=[0,0,0]
buttonpositions=[(19,170),(60,170),(19,220),(60,220),(19,270),(60,270),(19,320),(60,320),(19,370),(60,370),(19,420),(60,420),(400,430),(550,430),(460,207),(540,207),(460,257),(540,257),(460,307),(540,307),(500,357)]
makebuttons=['F','F\'','B','B\'','L','L\'','R','R\'','U','U\'','D','D\'','Scramble','Solve','Step 1','Step 2','Step 3','Step 4','Step 5','Step 6','Step 7']
buttons=[pygame.image.load(i+'.png') for i in makebuttons]
buttonrects=[pygame.Rect(buttonpositions[i][0],buttonpositions[i][1],buttons[i].get_width(),buttons[i].get_height()) for i in range(len(buttons))]
overlay=pygame.Surface(dimensions)
overlay.set_colorkey((255,0,0))
overlay.fill((255,0,0))
stepfunction={14:step1,15:step2,16:step3,17:step4,18:step5,19:step6,20:step7}
aftersolvetimer=pygame.time.Clock()
#Fbutton=pygame.Surface(dimensions)
#Fbutton.fill((0,255,0))
lastpress=False
aftersolvefont=pygame.font.Font('Arial Bold.ttf',40)
aftersolvetiming=0
arrowkeyfont=pygame.font.Font('Arial Bold.ttf',10)
watermarkfont=pygame.font.Font('Arial Bold.ttf',45)
watermark=pygame.Surface(watermarkfont.size('Made by Andrew Ogundimu'))
pygame.Surface.convert(watermark)
watermark.set_alpha(64)
watermark.set_colorkey((0,0,0))
watermark.blit(watermarkfont.render('Made by Andrew Ogundimu',True,(255,255,255)),(0,0))
showsolve=False
solvespeed=5
arrowsurf=pygame.Surface((50,25))
arrowsurf.fill((0,0,0))
arrowsurf.set_colorkey((0,0,0))
pygame.draw.polygon(arrowsurf,(255,255,255),[(0,12.5),(12.5*math.sqrt(3),0),(12.5*math.sqrt(3),25)])
pygame.draw.polygon(arrowsurf,(255,255,255),[(50-12.5*math.sqrt(3),12.5),(50,0),(50,25)])
arrowtimer=pygame.time.Clock()
arrowtick=0
arrowshow=0
basespeed=5
animspeed=basespeed
axisangles=[[0,0] for i in range(3)]
vectoradd=[0,0,0]
while True:
    #print(finishturn,rubiksmovequeue)
    aftersolvetiming+=aftersolvetimer.tick()
    if aftersolvetiming>5000:
        pygame.draw.rect(overlay,(255,0,0),pygame.Rect(0,0,dimensions[0],100))
        showsolve=False
        ...
        if event.type==pygame.KEYDOWN:
            ...
            if event.key==K_f:
                if fullscreen==False:
                    ...
                    fullscreen=False
                if fullscreen==True:
                    window = pygame.display.set_mode(dimensions, DOUBLEBUF | OPENGL|FULLSCREEN)
                    gluPerspective(45, dimensions[0] / dimensions[1], 0.1, 2000 * 3 / size)
                    glTranslatef(0, -2, -30)
    ...
    mp=[(pygame.mouse.get_pos()[0]-dimensions[0]/2)/(min(dimensions)/2),-(pygame.mouse.get_pos()[1]-dimensions[1]/2)/(min(dimensions)/2)]
    if math.hypot(*mp)>1:
        mp=[math.cos(math.atan2(mp[1],mp[0])),math.sin(math.atan2(mp[1], mp[0]))]
    mp=(mp[0]+vectoradd[0],mp[1]+vectoradd[1],math.sqrt(abs(1-(mp[0]**2+mp[1]**2)))+vectoradd[2])
    smp=[(pygame.mouse.get_pos()[0]-mousemovement[0]-dimensions[0]/2)/(min(dimensions)/2),-(pygame.mouse.get_pos()[1]-mousemovement[1]-dimensions[1]/2)/(min(dimensions)/2)]
    if math.hypot(*smp)>1:
        smp=[math.cos(math.atan2(smp[1],smp[0])),math.sin(math.atan2(smp[1], smp[0]))]
    smp=(smp[0]+vectoradd[0],smp[1]+vectoradd[1],math.sqrt(abs(1-(smp[0]**2+smp[1]**2)))+vectoradd[2])
    crossproduct=(mp[1]*smp[2]-mp[2]*smp[1],mp[2]*smp[0]-mp[0]*smp[2],mp[0]*smp[1]-mp[1]*smp[0])
    if pygame.mouse.get_pressed()[0]:
        glRotate(math.hypot(*mousemovement), *crossproduct)
        for i in range(len(vectoradd)):
            vectoradd[i]+=crossproduct[i]
        #print(mousemovement, [round(i,2) for i in mp],[round(x,2) for x in smp],crossproduct)
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
    background()
    print(math.hypot(*mousemovement),*crossproduct)
    glPushMatrix()
    RubiksCube()
    if finishturn==True and len(rubiksmovequeue)>0:
        del rubiksmovequeue[0]
    #print(finishturn,len(rubiksmovequeue))
    overlay=pygame.image.fromstring(glReadPixels(0,0,dimensions[0],dimensions[1],GL_RGB,GL_UNSIGNED_BYTE),dimensions,'RGB',True)
    for i in buttons:
        overlay.blit(i,buttonpositions[buttons.index(i)])
    for i in buttonrects:
        darken = pygame.Surface((i.w, i.h))
        pygame.Surface.convert(darken)
        darken.set_alpha(128)
        darken.fill((0, 0, 0))
        if i.collidepoint(pygame.mouse.get_pos()) and pygame.mouse.get_pressed()[0]:
            overlay.blit(darken, (i.x, i.y))
        if i.collidepoint(pygame.mouse.get_pos()) and pygame.mouse.get_pressed()[0] and pygame.mouse.get_pressed()[0]!=lastpress:
            if buttonrects.index(i)<=11:
                rubiksmovequeue.append(makebuttons[buttonrects.index(i)])
            if buttonrects.index(i)==13 and solve==False:
                if len(solve_(allcolors))>0:
                    rubiksmovequeue=solve_(allcolors)
                    solve=True
                    solvetime = time.time()
                    animspeed=solvespeed
            if buttonrects.index(i)>13:
                rubiksmovequeue=stepfunction[buttonrects.index(i)](allcolors)[0]
            if buttonrects.index(i)==12:
                rubiksmovequeue = []
                for i in range(20):
                    rubiksmovequeue.append(
                        possiblemoves[random.randint(0, 5)] + possibleextensions[random.randint(0, 2)])
                    if len(rubiksmovequeue) > 1:
                        while rubiksmovequeue[len(rubiksmovequeue) - 1][0] == rubiksmovequeue[len(rubiksmovequeue) - 2][
                            0]:
                            rubiksmovequeue[len(rubiksmovequeue) - 1] = possiblemoves[random.randint(0, 5)] + \
                                                                        possibleextensions[random.randint(0, 2)]
    if finishturn==True and len(rubiksmovequeue)==0:
        if solve==True:
            aftersolvetiming=0
    ...
    if showsolve==True:
        overlay.blit(
            aftersolvefont.render('Solve took %f seconds' % (finaltime - solvetime), True, (255, 255, 255)),
            ((dimensions[0] - aftersolvefont.size('Solve took %f seconds' % (finaltime - solvetime))[0]) / 2, 20))
    queueremove=False
    glPopMatrix()
    #print(bytes(0xFF0000))
    arrowtick+=arrowtimer.tick()
    if arrowtick>500:
        arrowshow=0
        arrowtick=0
    if arrows[0] or arrowshow==1:
        overlay.blit(arrowsurf,(dimensions[0]/2-50,dimensions[1]-50))
        arrowshow=1
        if arrows[0]:
            solvespeed/=2
            basespeed/=2
            if solve==True:
                ...
                animspeed=basespeed
            arrowtick=0
    if arrows[1] or arrowshow==2:
        overlay.blit(pygame.transform.flip(arrowsurf,True,False), (dimensions[0] / 2 - 50, dimensions[1] - 50))
        arrowshow=2
        if arrows[1]:
            solvespeed*=2
            basespeed*=2
            if solve==True:
                ...
                animspeed=basespeed
            arrowtick=0
    if not(arrows[1] or arrowshow==2 or arrows[0] or arrowshow==1):
        overlay.blit(arrowkeyfont.render('Press Left and Right Arrows to change speed', True, (255,255,255)),(dimensions[0]/2-200,dimensions[1]-20))
    overlay.blit(watermark, ((dimensions[0] - watermarkfont.size('Made by Andrew Ogundimu')[0])/2, (dimensions[1] - watermarkfont.size('Made by Andrew Ogundimu')[1])/2))
    glEnable(GL_BLEND)
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
    glDrawPixels(dimensions[0],dimensions[1],GL_RGBA,GL_UNSIGNED_BYTE,pygame.image.tostring(overlay,'RGBA',True))
    glDisable(GL_BLEND)
    pygame.display.flip()
    overlay.fill((0,0,0))
    #print(solvespeed)
    #pygame.image.save(pygame.image.fromstring(glReadPixels(0,0,dimensions[0],dimensions[1],GL_RGB,GL_UNSIGNED_BYTE),dimensions,'RGB',True),'screenshot.jpg')
    lastpress=pygame.mouse.get_pressed()[0]