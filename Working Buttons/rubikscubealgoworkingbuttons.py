...
#August 8, 2019: Finished adding the buttons to the program.
#November 21, 2019: Added Hide GUI Button and fixed the Step buttons.
#other dates: (on Google Drive)
    ...
    if not(hidegui):
        ...
        for i in buttonrects:
            ...
            if i.collidepoint(pygame.mouse.get_pos()) and pygame.mouse.get_pressed()[0] and pygame.mouse.get_pressed()[0]!=lastpress:
                ...
                if buttonrects.index(i)==14:
                    rubiksmovequeue=stepfunction[buttonrects.index(i)](allcolors)[0]
                if buttonrects.index(i)>14:
                    currentfunc = stepfunction[14](allcolors)
                    for q in currentfunc[0]:
                        rubiksmovequeue.append(q)
                    for x in range(15,buttonrects.index(i)):
                        currentfunc=stepfunction[x](currentfunc[1])
                        for q in currentfunc[0]:
                            rubiksmovequeue.append(q)
                    currentfunc = stepfunction[buttonrects.index(i)](currentfunc[1])
                    for q in currentfunc[0]:
                        rubiksmovequeue.append(q)
                    #print(rubiksmovequeue)
