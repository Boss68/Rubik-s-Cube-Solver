import pygame, threading,math, random,time
...
up=True
rotatekey=[0,0,0]
FButton=pygame.image.load('GUI/F.bmp')
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type==pygame.MOUSEBUTTONDOWN:
            ...
            if event.button==5:
                glTranslatef(0,0,5)
            if event.button==1:
                if pygame.Rect(10,dimensions[1]/2,FButton.get_width(),40).collidepoint(pygame.mouse.get_pos()):
                    rubiksmovequeue.append('F')
    ...
    if running==True:
        ...
        glPushMatrix()
        glLoadIdentity()
        gluOrtho2D(0,dimensions[0],dimensions[1],0)
        glMatrixMode(GL_MODELVIEW)
        glPushMatrix()
        glLoadIdentity()
        glColor3f(1.0,1.0,1.0)
        glBegin(GL_QUADS)
        glVertex2f(10,dimensions[1]/2)
        glVertex2f(FButton.get_width()+10, dimensions[1] / 2)
        glVertex2f(FButton.get_width()+10, dimensions[1] / 2 + 40)
        glVertex2f(10, dimensions[1] / 2 + 40)
        glEnd()
        glMatrixMode(GL_PROJECTION)
        glPopMatrix()
        glMatrixMode(GL_MODELVIEW)
