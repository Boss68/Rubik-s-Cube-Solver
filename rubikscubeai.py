import pygame, threading,math, random, neat, os
from pygame.locals import *
from OpenGL.GL import *
from OpenGL.GLU import *

pygame.init()
#using NEAT-Python module
dimensions=(800,600)
arcballrad=min(dimensions)/2
...
def getturn():
    ...
    while True:
        if finishturn==True and scramble==False:
            #print(fitnesscalc(allcolors),allcolors)
...
#configuration=neat.config.Config(genome_type=neat.DefaultGenome,stagnation_type=neat.DefaultStagnation,reproduction_type=neat.DefaultReproduction,species_set_type=neat.DefaultSpeciesSet,filename=os.path.join(os.path.dirname(__file__),'rubiksconfig'))
#neatpop=neat.population.Population(configuration)
if scramble==False:
...
for a in range(size):
    for b in range(size):
        for c in range(size):
            ...
            if c==size-1:
                solved[(a, b, c)][2] = 4
def fitnesscalc(fitInput):
    global solved
    solvedlist=[]
    fitness=0
    for i in solved:
        solvedlist.append(solved[i])
    inputList=[]
    for i in fitInput:
        inputList.append(fitInput[i])
    for i in zip(solvedlist,inputList):
        if i[0]==i[1]:
            fitness+=10
        elif i[0]!=i[1] and sorted(i[0])==sorted(i[1]):
            fitness+=1
        elif i[0]!=i[1] and sorted(i[0])!=sorted(i[1]):
            fitness+=0
    fitness-=70
    return fitness
#winner=neatpop.run(fitnesscalc,10)
#print(winner)
print(fitnesscalc(allcolors))
#print(newallcolors)
...
p2=(0,0,0)
while running:
    movementmouse = pygame.mouse.get_rel()
    ...
    for event in pygame.event.get():
        ...
        if event.type==pygame.MOUSEBUTTONDOWN:
            ...
            if event.button==1:
                ...
                p1[0]-=dimensions[0]/2
                p1[1]-=dimensions[1]/2
                if p1[0] ** 2 + p1[1] ** 2 <= arcballrad ** 2:
                    p1.append(math.sqrt(arcballrad ** 2 - p1[0] ** 2 - p1[1] ** 2))
                else:
                    p1.append(0)
    ...
    if running==True:
        ...
        if pygame.mouse.get_pressed()[0]:
            ...
            p2[0] -= dimensions[0] / 2
            p2[1] -= dimensions[1] / 2
            if p2[0]**2+p2[1]**2<=arcballrad**2:
                p2.append(math.sqrt(arcballrad**2-p2[0]**2-p2[1]**2))
            else:
                p2.append(0)
            rotatekey[1]+=movementmouse[0]
            rotatekey[0]+=movementmouse[1]
        else:
            p1=(0,0,0)
            p2=(0,0,0)
        ...
        #glRotate(math.hypot(*movementmouse),rotatekey[0],rotatekey[1],0)
        RubiksCube()
        glPopMatrix()
        #print(p1,p2)
        if finishturn==True and len(rubiksmovequeue)>0:
